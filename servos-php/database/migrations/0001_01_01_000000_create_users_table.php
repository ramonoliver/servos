...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->enum('status', ['active', 'inactive', 'paused', 'vacation'])->default('active');
            $table->string('avatar_color')->default('#F4532A');
            $table->text('photo_url')->nullable();
            $table->unsignedBigInteger('spouse_id')->nullable();
            $table->json('availability')->nullable();
            $table->unsignedInteger('total_schedules')->default(0);
            $table->unsignedTinyInteger('confirm_rate')->default(100);
            $table->boolean('must_change_password')->default(false);
            $table->date('last_served_at')->nullable();
