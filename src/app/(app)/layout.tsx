...
function UserAvatar({ user, size = 32 }: { user: User; size?: number }) {
  if (user.photo_url) {
    return <img src={user.photo_url} alt={user.name} className="rounded-full object-cover flex-shrink-0" style={{ width: size, height: size }} />;
  }
  return (
    <div className="rounded-full flex items-center justify-center text-white font-bold flex-shrink-0"
      style={{ width: size, height: size, fontSize: size * 0.32, background: user.avatar_color }}>
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  const db = getDB();
  const unreadNotifs = db.getWhere<Notification>("notifications", { user_id: user.id }).filter(n => !n.read).length;
  ...
  const roleBadge = user.role === "admin" ? { label: "Admin", cls: "bg-purple-50 text-purple-600" }
    : user.role === "leader" ? { label: "Lider", cls: "bg-brand-light text-brand" }
    : { label: "Membro", cls: "bg-success-light text-success" };

  return (
    <div className="flex h-screen bg-bg overflow-hidden">
      {/* Sidebar */}
      <aside className={`${collapsed ? "w-0 overflow-hidden" : "w-[250px]"} bg-white border-r border-border-soft flex flex-col flex-shrink-0 transition-all duration-300 z-10`}>
          ...
          {nav.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`sidebar-item ${active ? "active" : ""}`}>
                ...
                {item.badge && <span className="text-[10px] font-bold bg-danger text-white px-1.5 py-px rounded-full min-w-[18px] text-center">{item.badge}</span>}
              </Link>
            );
          })}
          {(isAdmin || isLeader) && departments.length > 0 && (
            <>
              <div className="text-[9px] font-bold tracking-widest uppercase text-ink-faint px-3 pt-4 pb-1">Ministerios</div>
              {departments.map(d => (
                <Link key={d.id} href={`/ministerios/${d.id}`} className={`sidebar-item pl-4 ${pathname === `/ministerios/${d.id}` ? "active" : ""}`}>
        ...
        <div className="px-4 py-3 border-t border-border-soft">
          <div className="flex items-center gap-2.5">
            ...
            <button onClick={logout} className="text-ink-faint hover:text-danger transition-colors p-1" title="Sair">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-bg/90 backdrop-blur-xl border-b border-border-soft flex items-center px-6 gap-4 flex-shrink-0">
          ...
          <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full ${roleBadge.cls}`}>{roleBadge.label}</span>
          <Link href="/perfil">
            <UserAvatar user={user} size={32} />
          </Link>
        </header>
